...
from brapi.generated_common import ListType, ExternalReference, AdditionalInfo, ListType
...
class List(BaseModel):
    ...
    listOwnerPerson: Optional[Any] = None
    listType: Optional[Any] = None
...
def list_to_df(items: List[List]) -> pd.DataFrame:
    ...
    Nested sub-model lists (``) are serialised to JSON strings so each List remains
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def program_names(self, program_names: Union[str, List[str]]) -> "ListQuery":
