...
def scale_to_df(items: List[Scale]) -> pd.DataFrame:
    ...
    Nested sub-model lists (``) are serialised to JSON strings so each Scale remains
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def program_names(self, program_names: Union[str, List[str]]) -> "ScaleQuery":
