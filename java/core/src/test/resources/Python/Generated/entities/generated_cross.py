...
class Cross(BaseModel):
    ...
    plannedCross: Optional[Any] = None
...
def cross_to_df(items: List[Cross]) -> pd.DataFrame:
    ...
    Nested sub-model lists (``) are serialised to JSON strings so each Cross remains
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def program_names(self, program_names: Union[str, List[str]]) -> "CrossQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def crossing_project_names(self, crossing_project_names: Union[str, List[str]]) -> "CrossQuery":
