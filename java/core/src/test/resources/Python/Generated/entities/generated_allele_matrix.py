...
def allele_matrix_to_df(items: List[AlleleMatrix]) -> pd.DataFrame:
    ...
    Nested sub-model lists (``) are serialised to JSON strings so each AlleleMatrix remains
...
the starting position of the range, and &lt;end&gt; is the ending position of the range
    ...
    def germplasm_names(self, germplasm_names: Union[str, List[str]]) -> "AlleleMatrixQuery":
        """A list of human readable `Germplasm` names

        Example::
            client.allele_matrix
                .germplasm_names("a03202ec")
                .fetch()
                .to_df()
            
            client.allele_matrix
                .germplasm_names(["a03202ec", "274e4f63"])
                .fetch()
                .to_df()
