...
def person_to_df(items: list[Person]) -> pd.DataFrame:
    ...
    ``lists``,
    ``programs``) are serialised to JSON strings so each Person remains
...
Use `GET /commoncropnames` to find the list of available crops on a server.
    ...
    def by_program_db_id(self, program_db_ids: Union[str, List[str]]) -> "PersonQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def by_program_name(self, program_names: Union[str, List[str]]) -> "PersonQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def user_ids(self, user_ids: Union[str, List[str]]) -> "PersonQuery":
        ...
        """
        return self._set_param("userIDs", user_ids)  # type: ignore[return-value]
    ...
    def filter(
        ...
        *,
        ...
        if email_addresses:
            q = q.email_addresses(email_addresses)
        if first_names:
            q = q.first_names(first_names)
        if last_names:
            q = q.last_names(last_names)
        if mailing_addresses:
            q = q.mailing_addresses(mailing_addresses)
        if middle_names:
            q = q.middle_names(middle_names)
        if person_db_ids:
            q = q.person_db_ids(person_db_ids)
        if phone_numbers:
            q = q.phone_numbers(phone_numbers)
        if user_ids:
            q = q.user_ids(user_ids)
        return q
    ...
    def list(
        ...
        *,
        ...
        for search_key, value in self._params.items():
            ...
            if get_key is None:
                continue  # not supported on GET people
