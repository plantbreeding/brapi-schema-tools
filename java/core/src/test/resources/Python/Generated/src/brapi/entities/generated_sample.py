...
Use `GET /commoncropnames` to find the list of available crops on a server.
    ...
    def by_germplasm_db_id(self, germplasm_db_ids: Union[str, List[str]]) -> "SampleQuery":
    ...
    def by_program_db_id(self, program_db_ids: Union[str, List[str]]) -> "SampleQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def by_study_db_id(self, study_db_ids: Union[str, List[str]]) -> "SampleQuery":
    ...
    def by_trial_db_id(self, trial_db_ids: Union[str, List[str]]) -> "SampleQuery":
    ...
    def by_observation_unit_db_id(self, observation_unit_db_ids: Union[str, List[str]]) -> "SampleQuery":
    ...
    def by_plate_db_id(self, plate_db_ids: Union[str, List[str]]) -> "SampleQuery":
        """The ID which uniquely identifies a `Plate` of `Samples`

        Example::
            client.sample
                .by_plate_db_id("0cac98b8")
                .fetch()
                .to_df()
            
            client.sample
                .by_plate_db_id(["0cac98b8", "b96125fb"])
                .fetch()
                .to_df()
    ...
    def filter(
        ...
        *,
        ...
        if plate_db_ids:
            q = q.by_plate_db_id(plate_db_ids)
        if plate_names:
            q = q.by_plate_name(plate_names)
        if sample_db_ids:
            q = q.sample_db_ids(sample_db_ids)
        if sample_names:
