...
class MarkerPositionQuery(BaseQuery[MarkerPosition]):
    ...
    pattern) so the same base query can be forked::
        ...
        base = client.marker_position.by_map_dbid("7e6fa8aa")
        q1   = base.linkage_group_names("Chromosome 2")
        q2   = base.by_variant_dbid("a0caa928")
    You can provide single values one at time or a list to each filter method, for example::
        q3 = client.marker_position.by_map_dbid("7e6fa8aa").by_map_dbid("bedc418c")  # one value as time
        q4 = client.marker_position.by_map_dbid(["7e6fa8aa", "bedc418c", ...])  # list
    ...
    def by_map_dbid(self, map_dbids: Union[str, List[str]]) -> "MarkerPositionQuery":
        """A list of IDs which uniquely identify `GenomeMaps` within the given database server

        Example::
            client.marker_position
                .by_map_dbid("7e6fa8aa")
                .fetch()
                .to_df()
            
            client.marker_position
                .by_map_dbid(["7e6fa8aa", "bedc418c"])
                .fetch()
                .to_df()
    ...
    def by_variant_dbid(self, variant_dbids: Union[str, List[str]]) -> "MarkerPositionQuery":
        """A list of IDs which uniquely identify `Variants` within the given database server

        Example::
            client.marker_position
                .by_variant_dbid("a0caa928")
                .fetch()
                .to_df()
            
            client.marker_position
                .by_variant_dbid(["a0caa928", "f8894a26"])
                .fetch()
                .to_df()
