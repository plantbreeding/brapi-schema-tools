...
Use `GET /commoncropnames` to find the list of available crops on a server.
    ...
    def by_program_dbid(self, program_dbids: Union[str, List[str]]) -> "ListQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def filter(
        ...
        *,
        ...
        if list_dbids:
            q = q.list_dbids(list_dbids)
        if list_names:
            q = q.list_names(list_names)
        if list_owner_names:
            q = q.list_owner_names(list_owner_names)
        if list_owner_person_dbids:
            q = q.list_owner_person_dbids(list_owner_person_dbids)
        if list_sources:
