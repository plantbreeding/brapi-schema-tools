...
class TrialQuery(BaseQuery[Trial]):
    ...
    pattern) so the same base query can be forked::
        ...
        q1   = base.by_location_dbid("b28911cf")
...
Use `GET /commoncropnames` to find the list of available crops on a server.
    ...
    def by_location_dbid(self, location_dbids: Union[str, List[str]]) -> "TrialQuery":
    ...
    def by_observation_variable_dbid(self, observation_variable_dbids: Union[str, List[str]]) -> "TrialQuery":
    ...
    def by_program_dbid(self, program_dbids: Union[str, List[str]]) -> "TrialQuery":
...
Use `GET /programs` to find the list of available programs on a server.
    ...
    def by_study_dbid(self, study_dbids: Union[str, List[str]]) -> "TrialQuery":
...
- `searchDateRangeEnd` is after `trial.startDate` AND `searchDateRangeStart` is before `trial.endDate`
    ...
    def filter(
        ...
        *,
        ...
        if trial_dbids:
            q = q.trial_dbids(trial_dbids)
        if trial_names:
            q = q.trial_names(trial_names)
        if active:
            q = q.active(active)
        if contact_dbids:
            q = q.contact_dbids(contact_dbids)
