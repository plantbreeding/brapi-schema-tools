-- ObservationUnitAttribute

/* 
Observation Unit Attributes are characteristics (characterization descriptors) for which they are Values 
...
Observation Unit Attributes are Variables with the Variable type 'Variate'.
 ...
 */
CREATE TABLE IF NOT EXISTS sta_dash.dadi_br_sandbox.silver_phenome_observation_unit_attributes (
...
COMMENT 'Observation Unit Attributes are characteristics (characterization descriptors) for which they are Values  associated with an Observation Unit but are not Observations. Typically,  they are characteristics known before the start of the  trial or study.   ### Phenome Implementation  Observation Unit Attributes are Variables with the Variable type \\'Variate\\'.  ### PRISM Implementation  > TODO Breeding Services please check  In Field Crops and Vegetable Observation Variables are held different workbooks depending on the type of Study and are columns in the corresponding field book table.';


/* 
Controlled Vocabulary for attributeCategories of ObservationUnitAttribute
 */
...
USING delta
TBLPROPERTIES ('delta.enableChangeDataFeed' = true)
COMMENT 'Controlled Vocabulary table for property attributeCategory on ObservationUnitAttribute';
