...
CREATE TABLE brapi_Observations (
  ...
  observationUnitDbId STRING COMMENT 'The ID which uniquely identifies an observation unit. The ID which uniquely identifies an observation unit  MIAPPE V1.1 (DM-70) Observation unit ID - Identifier used to identify the observation unit in data files containing the values observed or measured on that unit. Must be locally unique.',
  observationUnitPUI STRING COMMENT 'The ID which uniquely identifies an observation unit. A Permanent Unique Identifier for an observation unit  MIAPPE V1.1 (DM-72) External ID - Identifier for the observation unit in a persistent repository, comprises the name of the repository and the identifier of the observation unit therein. The EBI Biosamples repository can be used. URI are recommended when possible.',
  observationUnitName STRING COMMENT 'The ID which uniquely identifies an observation unit. A human readable name for an observation unit',
  observationVariableDbId STRING COMMENT 'The ID which uniquely identifies an observation variable. Variable unique identifier  MIAPPE V1.1 (DM-83) Variable ID - Code used to identify the variable in the data file. We recommend using a variable definition from the Crop Ontology where possible. Otherwise, the Crop Ontology naming convention is recommended: <trait abbreviation>_<method abbreviation>_<scale abbreviation>). A variable ID must be unique within a given investigation.',
  observationVariablePUI STRING COMMENT 'The ID which uniquely identifies an observation variable. The Permanent Unique Identifier of a Observation Variable, usually in the form of a URI',
  observationVariableName STRING COMMENT 'The ID which uniquely identifies an observation variable. Variable name (usually a short name)  MIAPPE V1.1 (DM-84) Variable name - Name of the variable.',
  seasonDbId STRING COMMENT 'The season for this Observation. The ID which uniquely identifies a season. For backward compatibility it can be a string like ''2012'', ''1957-2004''',
  seasonPUI STRING COMMENT 'The season for this Observation',
  seasonName STRING COMMENT 'The season for this Observation. Name of the season. ex. ''Spring'', ''Q2'', ''Season A'', etc.',
