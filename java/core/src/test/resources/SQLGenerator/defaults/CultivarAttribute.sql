...
/* 
Cultivar "attributes" are simply-inherited characteristics (characterization descriptors) that are not environment-dependent but 
 ...
 */
CREATE TABLE IF NOT EXISTS sta_dash.dadi_br_sandbox.silver_phenome_cultivar_attributes (
...
COMMENT 'Cultivar "attributes" are simply-inherited characteristics (characterization descriptors) that are not environment-dependent but  inherent in the cultivar line.  They include genes, QTLs, and genetic "traits" like wheat grain  hardness.  They are often evaluated by genotyping for diagnostic markers.';


/* 
Controlled Vocabulary for attributeCategories of CultivarAttribute
 */
...
USING delta
TBLPROPERTIES ('delta.enableChangeDataFeed' = true)
COMMENT 'Controlled Vocabulary table for property attributeCategory on CultivarAttribute';
